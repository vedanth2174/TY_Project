import { opGenesisState, opHardforkFromString, l1GenesisState, l1HardforkFromString, } from "@nomicfoundation/edr";
...
import { EDR_NETWORK_REVERT_SNAPSHOT_EVENT, OPTIMISM_CHAIN_TYPE, } from "../../../constants.js";
...
import { isDebugTraceResult, isEdrProviderErrorData, } from "./type-validation.js";
import { clientVersion } from "./utils/client-version.js";
import { ConsoleLogger } from "./utils/console-logger.js";
import { edrRpcDebugTraceToHardhat, hardhatMiningIntervalToEdrMiningInterval, hardhatMempoolOrderToEdrMineOrdering, hardhatHardforkToEdrSpecId, hardhatAccountsToEdrOwnedAccounts, hardhatForkingConfigToEdrForkConfig, } from "./utils/convert-to-edr.js";
...
export class EdrProvider extends BaseProvider {
     ...
     */
    static async create({ chainDescriptors, networkConfig, loggerConfig = { enabled: false }, tracingConfig = {}, jsonRpcRequestWrapper, coverageConfig, }) {
        const printLineFn = loggerConfig.printLineFn ?? printLine;
        const replaceLastLineFn = loggerConfig.replaceLastLineFn ?? replaceLastLine;
        const providerConfig = await getProviderConfig(networkConfig, coverageConfig, chainDescriptors);
        let edrProvider;
    ...
    async request(requestArguments) {
        ...
        else if (jsonRpcRequest.method === "debug_traceTransaction" ||
            jsonRpcRequest.method === "debug_traceCall") {
            assertHardhatInvariant(isDebugTraceResult(jsonRpcResponse.result), "Invalid debug trace response");
    ...
    async #handleEdrResponse(edrResponse) {
        ...
        if (isFailedJsonRpcResponse(jsonRpcResponse)) {
            ...
            if (stackTrace !== null) {
                ...
                error = createSolidityErrorWithStackTrace(responseError.message, stackTrace, responseError.data.data, responseError.data.transactionHash);
            }
            else {
                error =
...
export async function getProviderConfig(networkConfig, coverageConfig, chainDescriptors) {
    const specId = hardhatHardforkToEdrSpecId(networkConfig.hardfork, networkConfig.chainType);
    const ownedAccounts = await hardhatAccountsToEdrOwnedAccounts(networkConfig.accounts);
