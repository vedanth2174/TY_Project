...
import { markTestRunDone, markTestRunStart, markTestWorkerDone, } from "../coverage/helpers.js";
...
const runSolidityTests = async ({ testFiles, chainType, grep, noCompile, verbosity }, hre) => {
    ...
    if (testFiles.length > 0) {
        rootFilePaths = testFiles.map((f) => resolveFromRoot(hre.config.paths.root, f));
    }
    else {
        ...
        rootFilePaths = (await Promise.all([
            getAllFilesMatching(hre.config.paths.tests.solidity, (f) => f.endsWith(".sol")),
    ...
    edrArtifacts.forEach(({ userSourceName, edrAtifact }) => {
        if (rootFilePaths.includes(resolveFromRoot(hre.config.paths.root, userSourceName)) &&
    ...
    const testSuiteIds = edrArtifacts
        .filter(({ userSourceName }) => rootFilePaths.includes(resolveFromRoot(hre.config.paths.root, userSourceName)))
    ...
    const config = solidityTestConfigToSolidityTestRunnerConfigArgs(chainType, hre.config.paths.root, solidityTestConfig, verbosity, observabilityConfig, grep);
    const tracingConfig = {
        buildInfos,
