...
import { exists, getAllDirectoriesMatching, getAllFilesMatching, readJsonFile, remove, writeJsonFile, writeJsonFileAsStream, writeUtf8File, } from "@nomicfoundation/hardhat-utils/fs";
...
export class SolidityBuildSystemImplementation {
    ...
    #options;
    #compileCache = {};
    #defaultConcurrency = Math.max(os.cpus().length - 1, 1);
    ...
    async build(rootFilePaths, options) {
        ...
        const { compilationJobsPerFile, indexedIndividualJobs } = compilationJobsResult;
        ...
        const uncachedSuccessfulResults = uncachedResults.filter((result) => !this.#hasCompilationErrors(result.compilerOutput));
    ...
    async getCompilationJobs(rootFilePaths, options) {
        ...
        await Promise.all(subgraphsWithConfig.map(async ([config, subgraph]) => {
            ...
            assertHardhatInvariant(subgraph.getRoots().size === 1, "individual subgraph doesn't have exactly 1 root file");
    ...
    async runCompilationJob(runnableCompilationJob, options) {
        ...
        const output = await compiler.compile(await runnableCompilationJob.getSolcInput());
    ...
    async emitArtifacts(runnableCompilationJob, compilerOutput) {
        ...
        const buildInfoPath = path.join(this.#options.artifactsPath, `build-info`, `${buildInfoId}.json`);
        const buildInfoOutputPath = path.join(this.#options.artifactsPath, `build-info`, `${buildInfoId}.output.json`);
    ...
    async cleanupArtifacts(rootFilePaths) {
        ...
        for (const file of await getAllDirectoriesMatching(this.#options.artifactsPath, (d) => d.endsWith(".sol"))) {
            const relativePath = path.relative(this.#options.artifactsPath, file);
            const testDirectorySubpath = path.relative(this.#options.projectRoot, this.#options.solidityTestsPath);
            const hasTestFileExtension = file.endsWith(".t.sol");
            const isInsideTestFolder = relativePath.startsWith(testDirectorySubpath + path.sep);
        ...
        const artifactPaths = await getAllFilesMatching(this.#options.artifactsPath, (p) => p.endsWith(".json") && // Only consider json files
            // Ignore top level json files
            p.indexOf(path.sep, this.#options.artifactsPath.length + path.sep.length) !== -1, (dir) => dir !== buildInfosDir);
        ...
        const duplicatedNames = [...artifactNameCounts.entries()]
            ...
            .map(([name, _]) => name);
        const duplicatedContractNamesDeclarationFilePath = path.join(this.#options.artifactsPath, "artifacts.d.ts");
    ...
    async compileBuildInfo(_buildInfo, _options) {
    ...
    async #cacheCompilationResult(indexedIndividualJobs, result, emitArtifactsResult, isolated) {
        ...
        for (const rootFilePath of rootFilePaths) {
            ...
            assertHardhatInvariant(typeFilePath !== undefined, `No type file found on map for ${rootFilePath}`);
    ...
    #printSolcErrorsAndWarnings(errors) {
        ...
        for (const error of errors) {
            if (error.severity === "error") {
                ...
                console.warn((error.formattedMessage ?? error.message).replace(/^\w+:/, (t) => chalk.yellow.bold(t)));
            }
        }
        const hasConsoleErrors = errors.some((e) => this.#isConsoleLogError(e));
        if (hasConsoleErrors) {
    ...
    async #printCompilationResult(runnableCompilationJobs) {
        ...
        for (const solcVersion of [...jobsPerVersionAndEvmVersion.keys()].sort()) {
            ...
            for (const evmVersion of [...jobsPerEvmVersion.keys()].sort()) {
                ...
                console.log(`Compiled ${rootFiles} Solidity ${pluralize("file", rootFiles)} with solc ${solcVersion} (evm target: ${evmVersion})`);
            }
        }
    }
}
